...
Plugin URI: https://www.wptools.tube/tube-video-curator-plugin
Author: .TUBE gTLD
...
class Tube_Video_Curator {    
  ...
  private static function create_classes() {
    ...
    // Get the Tube_YouTube_Duration instance [FUTURE]
    // require_once TUBE_VIDEO_CURATOR_DIR . '/classes/class-tube-vc-youtube-duration.php';
  ...
  function get_all_terms_for_tube_video_post_type_via_ajax( ) { // Returns an array of categories for a post type
    ...
    $terms = $this -> get_all_terms_for_tube_video_post_type(  );
  ...
  function create_tube_video_post( $args ){ // Setting and calling wp_insert_post();
      ...
      $author = apply_filters( 'tube_vc_filter_import_author', $author, $source_site, $creator_name );
      ...
      $post_type = apply_filters( 'tube_vc_filter_import_post_type', $post_type, $source_site, $creator_name );
      ...
      $my_post = apply_filters( 'tube_vc_filter_post_pre_insert', $my_post, $source_site, $creator_name );
      ...
      // terms came in as a string
      if ( $source_term_ids && ! is_array( $source_term_ids ) ):        
        
        // explode them to an array
        $default_import_term_ids = explode( ',', $source_term_ids );
          
      else:
        
        // get the default from plugin settings
        $default_import_term_ids = array( intval( get_option( 'tube_vc_default_import_term') ) );
       
      endif;
      
      $import_term_ids = apply_filters( 'tube_vc_filter_import_term', $default_import_term_ids, $source_site, $creator_name );
      
      if ( $import_term_ids && is_array($import_term_ids) ):
        
        $import_terms_sanitized = array();
          
        // sanitize all the the terms
        foreach ( $import_term_ids as $import_term_id ):
          
          $import_term = get_term( intval( trim( $import_term_id ) ) );
          
          if ( $import_term ):
            
            $import_terms_sanitized[$import_term->taxonomy][] = $import_term->term_id;    
        ...
        // set the post terms
        foreach ( $import_terms_sanitized as $term_taxonomy => $import_terms_by_tax ):
          
          wp_set_post_terms( $post_ID, $import_terms_by_tax, $term_taxonomy );    
      ...
      if ( $tags ):
        ...
        $tag_import_taxonomy = get_option( 'tube_vc_default_tag_import_taxonomy');
        
        $tag_import_taxonomy = apply_filters( 'tube_vc_filter_tag_import_taxonomy', $tag_import_taxonomy, $source_site, $creator_name );
        
        if ( $tag_import_taxonomy ):
          ...
          wp_set_post_terms( $post_ID, $tags_sanitized, $tag_import_taxonomy ); 
      ...
      if( $image_url ):
        ...
        // try to get image via wp_remote_get
        $image_data_get = wp_remote_get($image_url);
        
        // see if there's image data
        if ( $image_data_get && ! is_wp_error( $image_data_get ) ):
          
          // get the body of the response
          $image_data = $image_data_get['body'];          
          ...
          $put_bytes = file_put_contents($file, $image_data);

...
class Tube_VC_Source {
  ...
  function set_source_properties( $post_id ){
    ...
    if ( array_key_exists('tube_source_term_ids', $post_meta) && ! empty( $post_meta['tube_source_term_ids'][0] ) ):
      
        // get a string of IDs
        $this -> term_ids = $post_meta['tube_source_term_ids'][0];
        
       // get_post_meta( $source_post_id, 'tube_source_term_ids', true );
        ...
        //if ( $source_term_ids ):
          ...
          // make it a string
          //$source_term_ids_list = ( $source_term_ids ) ? implode( ',', $source_term_ids ) : '';
